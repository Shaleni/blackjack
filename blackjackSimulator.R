...
#####Counting Cards####
#######################
#updates global runningCount and cardsDealt
countCard <- function(card){
  if(card %in% 2:6){
    runningCount <<- runningCount + 1
  } else if (names(card) %in% c(10, "J","Q","K","A")){
    runningCount <<- runningCount - 1
  }
...
s1 <- function(results,deck,terminate, strategy, cCards){
  if(strategy==1){
...
s21 <- function(results,deck,terminate, strategy, cCards){
  if(strategy==21){
    #play with split flag
    pws = F
    dDown = F
...
s2 <- function(results,deck,terminate, strategy, cCards){
  if(strategy==2){
    #play with split flag
    pws = F
    dDown = F
...
s4 <- function(results,deck,terminate, strategy, cCards){
  if(strategy==4){
    #play with split flag
    pws = F
    dDown = F
...
s5 <- function(results,deck,terminate, strategy, cCards){
  if(strategy==5){
    #play with split flag
    pws = F
    dDown = F
...
runSimulations <- function(x, cCards){
  ...
  } else if (x==21 || x==27 || x==28){
    results <- s21(results, deck, terminate, x, cCards)
  }
...
cat("Running Strategy 2_1 with Counting Cards \n")
finalResults <- rowSums(sapply(1:simRun,function(i) sapply(X=runSimulations(21, T),FUN="+")))
names(finalResults) <- c("BlackJack","OtherWin","Tie","Loss","Bust","TotalBet","AmtLeft")
finalResults
cat("\n")
...
cat("Running Strategy 2_1 with Doubling Down \n")
finalResults <- rowSums(sapply(1:simRun,function(i) sapply(X=runSimulations(27),FUN="+")))
names(finalResults) <- c("BlackJack","OtherWin","Tie","Loss","Bust","TotalBet","AmtLeft")
finalResults
cat("\n")
